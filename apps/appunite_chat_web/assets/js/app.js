...
const Hooks = {
  ...
  CharacterCount: {
    ...
    updateCount() {
      ...
      const countElement = this.el.parentElement.querySelector(".text-xs")
      if (countElement) {
        countElement.textContent = `${currentLength}/${maxLength}`
        if (remaining <= 10) {
          countElement.classList.add("text-warning")
          countElement.classList.remove("text-base-content/50")
        } else {
          countElement.classList.remove("text-warning")
          countElement.classList.add("text-base-content/50")
        }
      }
    }
  }
